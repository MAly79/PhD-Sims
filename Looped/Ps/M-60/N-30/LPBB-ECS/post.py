...
def find_Ns(filename):
    ...
    Nshear = N_data[2,3]
    Nthermo = N_data[3,3]
    return Nequil, Ncomp, Nshear, Nthermo
...
if __name__ == '__main__':
    ...
    comp_data = np.genfromtxt('log.lammps',comments=None, dtype=float, skip_header=(starts[1]+1), max_rows=(int(Ncomp/Nthermo)+1))
    comp_df = pd.DataFrame(comp_data)
    comp_df.to_csv('comp.csv')

    #comp_data = read_thermo('log.lammps', starts[1], Ncomp, Nthermo)
    shear_data = np.genfromtxt('log.lammps',comments=None, dtype=float, skip_header=(starts[2]+1), max_rows=(int(Nshear/Nthermo)+1))
