...
# step et ke pe epair temp c_melTemp c_wallTemp v_Fcatom v_Pcomp2 press pxx pyy pzz pxy pxz pyz c_Vir c_Vir[1] c_Vir[2] c_Vir[3] c_Vir[4] c_Vir[5] c_Vir[6] c_melPress c_wallPress v_melDens v_surfcov v_aveRg v_Vwall v_srate v_D v_bwzmax zhi c_fbwall[1] c_fbwall[3] c_ftwall[1] c_ftwall[3] c_ggbot[1] c_ggbot[3] c_ggtop[1] c_ggtop[3]
    ...
    zDDa,zDDt,zDDb,zDVp,zDTp,zDPp = zD_profs(r'C:\Users\klay_\OneDrive - Imperial College London\PhD\PhD-Sims\Profiles\Full\MDPBB-ECS1\1_V',3)
    zDDa5,zDDt5,zDDb5,zDVp5,zDTp5,zDPp5 = zD_profs(r'C:\Users\klay_\OneDrive - Imperial College London\PhD\PhD-Sims\Profiles\Full\MDPBB-ECS1\5_V',3)
    zDDa10,zDDt10,zDDb10,zDVp10,zDTp10,zDPp10 = zD_profs(r'C:\Users\klay_\OneDrive - Imperial College London\PhD\PhD-Sims\Profiles\Full\MDPBB-ECS1\10_V',3)
    ...
    T5, = ax1.plot(zDTp5[0],zDTp5[1], 'r-')
    D5, = ax1.plot(zDDa5[0],zDDa5[1], 'g-')
    V5, = ax2.plot(zDVp5[0],zDVp5[1], 'b-')
    T10, = ax1.plot(zDTp10[0],zDTp10[1], 'r-o')
    D10, = ax1.plot(zDDa10[0],zDDa10[1], 'g-o')
    V10, = ax2.plot(zDVp10[0],zDVp10[1], 'b-o')    
    
    plt.legend([T1,V1,D1], [r'Temperature','Velocity','Density'], loc=4)
    ax1.set(xlim=[0,1],ylim=[0,2.25],xlabel='$z/D$', ylabel= r'$T, \phi(z)$')
    ax2.set(ylim=[-0.75,0.75], ylabel= r'V')
    ax1.minorticks_on()
    ax2.minorticks_on()
    ...
    Pzx5, = ax1.plot(zDPp5[0],zDPp5[1][:,0], 'r-')
    Pzz5, = ax1.plot(zDPp5[0],zDPp5[1][:,2], 'b-')
    D5, = ax2.plot(zDDa5[0],zDDa5[1], 'g-')    
    Pzx10, = ax1.plot(zDPp10[0],zDPp10[1][:,0], 'r-o')
    Pzz10, = ax1.plot(zDPp10[0],zDPp10[1][:,2], 'b-o')
    D10, = ax2.plot(zDDa10[0],zDDa10[1], 'g-o')
    ...
    Vs1,Cs1 = read_thermo(r'C:\Users\klay_\OneDrive - Imperial College London\PhD\PhD-Sims\Profiles\Full\MDPBB-ECS1',[13,15,30],3)    
    ...
    ax1.minorticks_on()
    ax2.minorticks_on()
    plt.savefig('7-PzzPxz vs srate - vV.jpg')
    ...
    plt.legend([l1], [r'P=1',], loc=1)     
    ax1.set(xlabel=r'$\.\gamma$', ylabel= r'$\mu$', xlim=[0,0.3], ylim=[0,0.45])
    ax1.minorticks_on()
    plt.savefig('8-COF vs srate - vV.jpg')
